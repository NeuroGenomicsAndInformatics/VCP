...
dds.HiSeq <- DESeqDataSetFromMatrix(countData = HiSeq.gene.select, colData = pheno.HiSeq.select, design = ~ condition + QC.RIN + GENDER  + AGE_at_death)
...
write.csv(res.HiSeq,"C9ORF72vsCO_Age_Sex_RIN_DESeq_res.csv",quote = F)
write.csv(res.HiSeq.sig,"C9ORF72vsCO_Age_Sex_RIN_DESeq_sig_res.csv",quote = F)
...
res.HiSeq <- results(dds.HiSeq,contrast = c("condition", "FTD","CO"))
res.HiSeq.sig <- res.HiSeq[which(res.HiSeq$padj < 0.05),]
write.csv(res.HiSeq,"GRNvsCO_Age_Sex_RIN_DESeq_res.csv",quote = F)
write.csv(res.HiSeq.sig,"GRNvsCO_Age_Sex_RIN_DESeq_sig_res.csv",quote = F)
rm(pheno.HiSeq.select,HiSeq.gene.select,dds.HiSeq,cts,geoMeans,res.HiSeq,res.HiSeq.sig)
...
res.HiSeq <- results(dds.HiSeq,contrast = c("condition", "FTD","CO"))
res.HiSeq.sig <- res.HiSeq[which(res.HiSeq$padj < 0.05),]
write.csv(res.HiSeq,"MAPTvsCO_Age_Sex_RIN_DESeq_res.csv",quote = F)
write.csv(res.HiSeq.sig,"MAPTvsCO_Age_Sex_RIN_DESeq_sig_res.csv",quote = F)
rm(pheno.HiSeq.select,HiSeq.gene.select,dds.HiSeq,cts,geoMeans,res.HiSeq,res.HiSeq.sig)
...
dds.HiSeq <- DESeqDataSetFromMatrix(countData = HiSeq.gene.select, colData = pheno.HiSeq.select, design = ~ condition)
cts <- counts(dds.HiSeq)
geoMeans <- apply(cts, 1, function(row) if (all(row == 0)) 0 else exp(mean(log(row[row != 0]))))
dds.HiSeq <- estimateSizeFactors(dds.HiSeq, geoMeans=geoMeans)
dds.HiSeq <- DESeq(dds.HiSeq)
res.HiSeq <- results(dds.HiSeq,contrast = c("condition", "FTD","CO"))
res.HiSeq.sig <- res.HiSeq[which(res.HiSeq$padj < 0.05),]
write.csv(res.HiSeq,"MAPTvsCO_DESeq_res.csv",quote = F)
write.csv(res.HiSeq.sig,"MAPTvsCO_DESeq_sig_res.csv",quote = F)
rm(pheno.HiSeq.select,HiSeq.gene.select,dds.HiSeq,cts,geoMeans,res.HiSeq,res.HiSeq.sig)
...
res.HiSeq <- results(dds.HiSeq,contrast = c("condition", "FTD","CO"))
res.HiSeq.sig <- res.HiSeq[which(res.HiSeq$padj < 0.05),]
write.csv(res.HiSeq,"FTDsporadicvsCO_Age_Sex_RIN_DESeq_res.csv",quote = F)
write.csv(res.HiSeq.sig,"FTDsporadicvsCO_Age_Sex_RIN_DESeq_sig_res.csv",quote = F)
rm(pheno.HiSeq.select,HiSeq.gene.select,dds.HiSeq,cts,geoMeans,res.HiSeq,res.HiSeq.sig)
